...
const resolvers = {
  ...
  Mutation: {
    ...
    createOrder: async (
      ...
      const saveImagesAndGetUrls = async (
          ...
          images.map(async ({ base64, contentType }, index) => {
            try {
              ...
              if (!result || !result.buffer || !result.contentType) {
                throw new Error(
                  `Invalid result from base64ToBuffer for image ${index}`
                )
              }
              const { buffer, contentType: type } = base64ToBuffer(
                base64,
                contentType
              )
              ...
              console.log(
                'inside: ',
                `https://storage.cloud.google.com/${bucket.name}/${fileName}`
              )
              ...
              if (error instanceof Error) {
                ...
                console.error(
                  'An unknown error occurred in saveImagesAndGetUrls.'
                )
              }
            }
          })
