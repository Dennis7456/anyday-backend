...
async function initialize() {
  try {
    ...
    const { GraphQLClient: ImportedGraphQLClient } = await import(
      'graphql-request'
    )
    ...
    console.error(
      'Error initializing GraphQL client:',
...
async function app() {
  ...
  server.route({
    ...
    handler: async (req, resp) => {
      ...
      if (!query) {
        resp
          .status(400)
          .send({ errors: [{ message: 'Must provide query string.' }] })
        return
      }
  ...
  server.addHook('preParsing', (request, reply, payload, done) => {
    if (
      request.raw.url === '/webhooks/stripe' &&
